...
| B-2 | med | Galeria as a further marketplace for `SCH-173983-803` | [agent] | Galeria carries the exact EAN 4007065791955; `og:price:amount` extraction works. Needs a `matches.csv` row + one test scrape. |
...
| B-7 | low | Highlight the cheapest marketplace per product in the overview table | [plan] | Phase 4 "cheapest rival highlighted" — only meaningful once a product has ≥ 2 marketplaces. |
